...
function generateConsolidatedReport() {
    ...
    logs.forEach((log, index) => {
        ...
        if (log.type === 'netcontent') {
            ...
            const vNos = p.nos && p.nos.length ? p.nos : Array(10).fill('');
            const vDests = p.dests && p.dests.length ? p.dests : Array(10).fill('');
            const vCleans = p.cleans && p.cleans.length ? p.cleans : Array(10).fill('Yes');
            const vOdours = p.odours && p.odours.length ? p.odours : Array(10).fill('Yes');
            const vTarps = p.tarps && p.tarps.length ? p.tarps : Array(10).fill('Yes');
            const vRemarks = p.remarks && p.remarks.length ? p.remarks : Array(10).fill('');
            const vInspectedBys = p.inspectedBys && p.inspectedBys.length ? p.inspectedBys : Array(10).fill('');
            const vSigns = p.signs && p.signs.length ? p.signs : Array(10).fill('');
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 0.6rem; margin-top: 0.5rem;">
                    <thead>
                        <tr style="background: #f8fafc;">
                            ...
                            ${vNos.map((_, i) => `<th style="border: 1px solid #cbd5e1; padding: 4px;">Veh ${i+1}</th>`).join('')}
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            ...
                            ${vNos.map(n => `<td style="border: 1px solid #cbd5e1; padding: 4px; text-align: center;">${n || '-'}</td>`).join('')}
                        </tr>
                        <tr>
                            ...
                            ${vDests.map(d => `<td style="border: 1px solid #cbd5e1; padding: 4px; text-align: center;">${d || '-'}</td>`).join('')}
                        </tr>
                        <tr>
                            ...
                            ${vCleans.map(c => `<td style="border: 1px solid #cbd5e1; padding: 4px; text-align: center;">${c || '-'}</td>`).join('')}
                        </tr>
                        <tr>
                            ...
                            ${vOdours.map(o => `<td style="border: 1px solid #cbd5e1; padding: 4px; text-align: center;">${o || '-'}</td>`).join('')}
                        </tr>
                        <tr>
                            ...
                            ${vTarps.map(t => `<td style="border: 1px solid #cbd5e1; padding: 4px; text-align: center;">${t || '-'}</td>`).join('')}
                        </tr>
                        <tr>
                            ...
                            ${vRemarks.map(r => `<td style="border: 1px solid #cbd5e1; padding: 4px; text-align: center;">${r || '-'}</td>`).join('')}
                        </tr>
                        <tr>
                            <td style="border: 1px solid #cbd5e1; padding: 4px; text-align: center;">-</td>
                            <td style="border: 1px solid #cbd5e1; padding: 4px;"><strong>Vehicle Inspected by</strong></td>
                            ${vInspectedBys.map(ib => `<td style="border: 1px solid #cbd5e1; padding: 4px; text-align: center;">${ib || '-'}</td>`).join('')}
                        </tr>
                        <tr>
                            <td style="border: 1px solid #cbd5e1; padding: 4px; text-align: center;">-</td>
                            <td style="border: 1px solid #cbd5e1; padding: 4px;"><strong>Sign.</strong></td>
                            ${vSigns.map(s => `<td style="border: 1px solid #cbd5e1; padding: 4px; text-align: center;">${s || '-'}</td>`).join('')}
                        </tr>
                    </tbody>
                </table>
            `;
